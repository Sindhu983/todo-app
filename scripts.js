...
function testy() {
    $('#add-box-cover').fadeOut(500);$('#add-box').fadeOut(500);
}
...
function addTodo() {
    if(newTodo.value == '' || newTodoTime.value == '') {
        ...
        if(newTodo.value == '')
        newTodo.style.borderBottomColor = 'red';

        if(newTodoTime.value == '')
        newTodoTime.style.borderBottomColor = 'red';
...
function done(a) {
    var targetP = $(a).parent().parent(); // targetting whole paragraph
